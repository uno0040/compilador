...
vector<string> analisadorLexical(ifstream &file){
    ...
    while (file>>noskipws>>c){
        ...
        if (isalpha(c)){
            ...
            if (tokenList.find(tokenValue) != tokenList.end()){
                ...
                tokens.push_back(tokenList[tokenValue]);
                return tokens;

            } else {
                token_count[tokenInd[tokenList["identificador"]]]++;
                tokens.push_back(tokenValue);
                tokens.push_back(tokenList["identificador"]);
                return tokens;
            } 
